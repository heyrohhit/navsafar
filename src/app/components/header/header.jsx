...
const Header = () => {
  ...
  // console.log("pathname is ", isActive)
  // console.log("mobile manu", mobileNav)
  return (
    <>
      {/* Desktop Header */}
      ...
      <header className="xl:hidden fixed top-0 left-0 right-0 z-50 bg-[#0F6177] backdrop-blur-xl border-b border-gray-200/50 shadow-sm p-2">
          ...
          {mobileNav.slice(0, 5).map((item, idx) => {
            return (
              <Link
                ...
                className={`flex flex-col items-center gap-1 transition-all duration-200 group ${isActive === item.path ? "text-blue-600" : "text-gray-600 hover:text-blue-600"
                  }`}
              >
                <div
                  className={`relative transition-all duration-300 ${isActive === item.path ? "transform scale-110 -translate-y-[3vh] bg-white shadow-[inset_0_4px_10px_rgba(0,0,0,0.5)] text-center rounded-full -shadow-[0_4px_10px_rgba(0,0,0,0.5)] transition-all" : "group-hover:scale-110"
                    }`}
                  style={isActive === item.path ? { padding: "12px" } : {}}
                >
                  {getTravelIcon(item.name)}
                  {isActive === item.path && (
                    <div className="absolute transition-all -bottom-1 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-blue-600 rounded-full -translate-y-[1vh]"></div>
                  )}
                </div>
                <span className={`text-xs font-medium ${isActive === item.path ? "  transition-all -translate-y-[2vh]" : ""}`}>
