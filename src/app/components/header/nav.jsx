      ...
      <div className="w-full h-full hidden xl:flex items-center justify-around text-white">
        ...
        {mainItems.map((item, idx) => (
          <Link
            key={idx}
            href={item.path}
            className={` relative flex  justify-around items-center hover:text-blue-600 transition-all duration-200 font-medium rounded-lg hover:bg-blue-50 whitespace-nowrap ${activeItem === item.path ? "text-[#000] bg-white" : "text-white"
              }`}
            title={item.description}
            aria-label={`${item.name} - ${item.description}`}
          >
