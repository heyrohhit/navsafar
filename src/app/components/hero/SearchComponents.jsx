// src/app/components/hero/SearchComponents.jsx
// ✅ FIXED: Shows SearchLeadPopup before navigating to results
"use client";
...
const SearchLeadPopup = dynamic(
  () => import("../search/SearchLeadPopup"),
  { ssr: false }
);
...
export default function SearchComponents() {
  ...
  // ── On "Search Packages" click — show popup first ────────
  ...
  // ── After popup success — navigate to search results ─────
  function handlePopupSuccess() {
    setShowPopup(false);
    if (!pendingSearch) return;
    const p = new URLSearchParams({
      q:          pendingSearch.q,
      from:       pendingSearch.from,
      date:       pendingSearch.date,
      travellers: pendingSearch.travellers.toString(),
      adults:     pendingSearch.adults.toString(),
      children:   pendingSearch.children.toString(),
      infants:    pendingSearch.infants.toString(),
    });
      ...
      <div className="w-full max-w-5xl mx-auto px-4 md:-translate-y-[20%] max-[780px]:translate-y-[-5%]" style={{ zIndex: 50, position: "relative" }}>
        ...
        <div className="bg-[#fff] text-[#0f6477] rounded-2xl p-5 shadow-2xl border border-white/10 backdrop-blur-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className="relative z-[999]">
              ...
              <input
                ...
                value={fromCity}
                onChange={(e) => { setFromCity(e.target.value); setFromSuggestions(getSuggestions(e.target.value)); setShowFromDrop(true); }}
                  ...
                  <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    ...
                    {fromSuggestions.map((loc, i) => (
                      <div key={i} onMouseDown={() => { setFromCity(loc.label); setShowFromDrop(false); }}
                        className="px-4 py-2.5 text-[#0f6477]/80 hover:bg-[#0f6477]/10 cursor-pointer text-sm border-b border-gray-100 last:border-none">
                        📍 {loc.label}
                      </div>
                    ))}
                  </motion.div>
            ...
            <div className="relative">
              ...
              <input
                ...
                value={destination}
                onChange={(e) => { setDestination(e.target.value); setDestSuggestions(getSuggestions(e.target.value)); setShowDestDrop(true); }}
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-[#0f6477] text-[#0f6477] outline-none focus:border-white/60 focus:bg-[#0f6477] transition-all text-sm focus:text-white"
            ...
            <div className="relative" ref={passengerRef}>
              ...
              <button type="button"
                ...
                className="w-full px-4 py-3 rounded-xl border border-[#0f6477] text-[#0f6477] outline-none focus:border-white/60 focus:bg-[#0f6477] transition-all text-sm text-left flex items-center justify-between">
                <span className="truncate">{passengerLabel}</span>
                <span className={`transition-transform duration-200 ${showPassenger ? "rotate-180" : ""}`}>▾</span>
              </button>
                ...
                {showPassenger && (
                  <motion.div initial={{ opacity: 0, y: -8, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.97 }}
                    ...
                    <div className="flex items-center justify-between">
                      <div><p className="text-white font-semibold text-sm">Adults</p><p className="text-white/50 text-xs">Age 12+</p></div>
                      <Counter value={adults} onChange={setAdults} min={1} max={10} />
                    </div>
                    <div className="border-t border-white/10" />
                    <div className="flex items-center justify-between">
                      <div><p className="text-white font-semibold text-sm">Children</p><p className="text-white/50 text-xs">Age 2–11</p></div>
                      <Counter value={children} onChange={setChildren} min={0} max={8} />
                    </div>
                    <div className="border-t border-white/10" />
                    <div className="flex items-center justify-between">
                      <div><p className="text-white font-semibold text-sm">Infants</p><p className="text-white/50 text-xs">Under 2</p></div>
          ...
          <button
            ...
            disabled={!destination.trim() && !fromCity.trim()}
            className="mt-4 w-full md:w-auto px-8 py-3.5 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all duration-300 hover:scale-105 shadow-lg shadow-green-500/30 text-sm disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
          >
            🔍 Search Packages
          </button>
        </div>
